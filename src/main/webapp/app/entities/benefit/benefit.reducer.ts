...
export const ACTION_TYPES = {
  SEARCH_BENEFITS: 'benefit/SEARCH_BENEFITS',
...
export default (state: BenefitState = initialState, action): BenefitState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_BENEFITS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_BENEFITS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_BENEFITS):
      return {
        ...state,
        loading: false,
...
const apiSearchUrl = 'api/_search/benefits';

// Actions

export const getSearchEntities: ICrudSearchAction<IBenefit> = query => ({
  type: ACTION_TYPES.SEARCH_BENEFITS,
  payload: axios.get<IBenefit>(`${apiSearchUrl}?query=` + query)
});
