...
describe('Country e2e test', () => {
  ...
  it('should load create Country page', async () => {
    ...
    expect(await countryUpdatePage.getPageTitle().getAttribute('id')).to.match(/talentPipeFrontendApp.country.home.createOrEditLabel/);
  });
  ...
  it('should create and save Countries', async () => {
    ...
    await countryUpdatePage.setNormalizedNameInput('normalizedName');
    ...
    await countryUpdatePage.setPhoneCodeInput('phoneCode');
    expect(await countryUpdatePage.getPhoneCodeInput()).to.match(/phoneCode/);
    await countryUpdatePage.setCurrencyInput('currency');
  ...
  it('should delete last Country', async () => {
    ...
    expect(await countryDeleteDialog.getDialogTitle().getAttribute('id')).to.match(/talentPipeFrontendApp.country.delete.question/);
