...
describe('Role e2e test', () => {
    ...
    it('should load Roles', async () => {
        ...
        expect(await roleComponentsPage.getTitle()).to.eq('talentpipeFrontendApp.role.home.title');
    });

    it('should load create Role page', async () => {
        await roleComponentsPage.clickOnCreateButton();
        roleUpdatePage = new RoleUpdatePage();
        expect(await roleUpdatePage.getPageTitle()).to.eq('talentpipeFrontendApp.role.home.createOrEditLabel');
    ...
    it('should create and save Roles', async () => {
        ...
        await roleComponentsPage.clickOnCreateButton();
        await promise.all([roleUpdatePage.setNameInput('name'), roleUpdatePage.setDescriptionInput('description')]);
        expect(await roleUpdatePage.getNameInput()).to.eq('name');
        expect(await roleUpdatePage.getDescriptionInput()).to.eq('description');
    ...
    it('should delete last Role', async () => {
        ...
        expect(await roleDeleteDialog.getDialogTitle()).to.eq('talentpipeFrontendApp.role.delete.question');
